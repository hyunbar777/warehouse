...
public class AppMain {
    ...
    public static void main(String[] args) {
        ...
        //生成数据
        genneratelog(delay, loop_len);
    }
    
    private static void genneratelog(long delay, int loop_len) {
        for (int i = 0; i < loop_len; i++) {
            int flag = rand.nextInt(2);
            switch (flag) {
                case (0):
                    //应用启动
                    String jsonString = JSON.toJSONString(generateStart());
    ...
    private static void putData() {
        ...
        String consoleLog = System.currentTimeMillis() + "|" + json.toJSONString();
